        ...
        >
                ...
                <div class="flex gap-2">
                  <div class="relative flex-1">
                    <input
                      ...
                      readonly
                      placeholder="請選擇公司..."
                      class="w-full pl-4 pr-4 py-3 border rounded-xl text-slate-700 outline-none cursor-default text-sm h-[46px] transition-colors"
                      :class="[
                        errors.customerId ? 'border-red-500 bg-red-50/30' : 'border-slate-200',
                        isEditing ? 'bg-slate-100 text-slate-400' : 'bg-slate-50'
                      ]"
                    >
                    <div v-if="isEditing" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300">
                  ...
                  <button
                    v-if="!isEditing"
                    type="button"
                    class="px-4 h-[46px] bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 transition shadow-sm flex items-center justify-center active:scale-95"
                    @click="$emit('open-selector')"
                  >
                    <i class="fa-solid fa-building text-indigo-500"></i>
                  </button>
                </div>
                <p v-if="errors.customerId" class="mt-1 text-sm text-red-500 font-medium">{{ errors.customerId }}</p>
                <p v-if="isEditing" class="mt-1 text-[11px] text-slate-400 italic">註：角色建立後不可變更所屬公司</p>
              </div>
          ...
          <div class="px-8 py-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <div class="text-xs text-slate-400 text-left">
              <span v-if="isEditing">最後更新日期：{{ formatDate(formData.updated_at) }}</span>
            </div>
...
export default {
  ...
  emits: ['close', 'submit', 'open-selector'],
  ...
  methods: {
    // 供父組件呼叫，設定選擇的公司
    setCustomer (company) {
      this.formData.customerId = company.id
      this.formData.customerName = company.name
    ...
    validate () {
      ...
      if (!this.formData.customerId) {
        this.errors.customerId = '請選擇所屬公司'
        isValid = false
      }
    ...
    async submitForm () {
      ...
      console.log('Submitting role data:', JSON.stringify(submitData, null, 2))
